...
namespace PdfSharp.Xamarin.Forms.Utils
{
    public class SizeUtils
    {
        ...
        public static XRect GetAvailablePageSize(PageSize pageSize)
        {
            //TODO implement for accurate margin
            var size = GetPageSize(pageSize);
            double margin = (size.Width + size.Height) * 0.05;
            return new XRect(margin, margin, size.Width - 2 * margin, size.Height - 2 * margin);
        }
    }
}
